...
## Resultado da hospedagem e contrato 0.3.2

A republicação 0.3.1 manteve application/octet-stream: o Sites entrega _headers como arquivo estático e não interpreta suas regras nessa modalidade de hospedagem. A tentativa e os dois resultados HTTP anteriores permanecem registrados acima. Não se afirma que o cabeçalho público foi corrigido.

O contrato passou a aceitar somente image/webp ou application/octet-stream, mantendo a verificação da assinatura RIFF/WEBP e do orçamento de bytes. O cenário mobile-touch verifica a decodificação real e as dimensões 1536×1024 em Chromium e WebKit. Isso valida o recurso que o visitante recebe, sem introduzir uma rota de servidor apenas para contornar um cabeçalho da plataforma. A configuração _headers é preservada para execução em hosts compatíveis, sem depender dela no Sites.

No WebKit público, a primeira execução do cenário de toque expirou esperando a posição da âncora do manual. A inspeção independente com fontes prontas confirmou o capítulo em y=92px, abaixo do índice de 50px, sem erros. O cenário agora aguarda hidratação e document.fonts.ready antes da navegação por âncora; essa prontidão faz parte da medição de geometria, sem alteração do app.
